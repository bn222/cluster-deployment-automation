...
def main_deploy_openshift(cc: ClustersConfig, args: argparse.Namespace, cdaConfig: CdaConfig) -> None:
    ...
    cd = ClusterDeployer(cc, ai, args.steps, args.secrets_path, cdaConfig, args.resume)
...
def main_deploy(args: argparse.Namespace, cdaConfig: CdaConfig) -> None:
    if cdaConfig:
    ...
    if cc.kind == "openshift":
        main_deploy_openshift(cc, args, cdaConfig)
    else:
        main_deploy_iso(cc, args)


def main_snapshot(args: argparse.Namespace, cdaConfig: CdaConfig) -> None:
    args = parse_args()
    cc = ClustersConfig(
        args.config,
        worker_range=args.worker_range,
...
def main_state(args: argparse.Namespace, conf: CdaConfig) -> None:
    s = StateFile(conf)
    if args.reset_state:
        s.clear_state()
        return
...
def is_yaml(config: str) -> bool:
    return bool(config) and (config.endswith('.yaml') or config.endswith('.yml'))
...
def main() -> None:
    ...
    if args.subcommand == "deploy":
        main_deploy(args, conf)
    elif args.subcommand == "snapshot":
        main_snapshot(args, conf)
