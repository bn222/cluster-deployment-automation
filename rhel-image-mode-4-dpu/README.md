...
- To explain why RHEL Image Mode is suitable for DPU deployments.
- To document the steps taken to get a RHEL Bootc image running on the DPU.
- To highlight pain points encountered and the solutions that resolved them.

---
## Kickstart Deployment Workflow
![RHEL-Image-Mode-Kickstart](img/rhel-image-mode-for-dpu-kickstart-deploy.png)
## Bootc-image-builder Deployment Workflow
...
4. Build the container and push it to a registry and run the `ipu-rhel-iso-builder` container:
    - **Step 1**: Build the bootc image with podman:
    ```bash
      export BOOTC_IMAGE_URL=quay.io/sadasilv/ipu-microshift-rhel:10
      export DESIRED_ARCH=aarch64
      ```

    - Build the bootc image with podman and pushed remotely:
      ```bash
      sudo podman build \
    ...
    - Optional: Build the bootc image with podman if not already built and pushed remotely:
      ```bash
      sudo podman build \
      --security-opt label=type:unconfined_t \
      --platform linux/arm64 \
