...
class ClusterNode:
    ...
    def wait_for_boot(self, desired_ip_range: tuple[str, str]) -> None:
        common.wait_true(f"{self.config.name} boot", self.has_booted)
        common.wait_true(f"{self.config.name} post_boot", self.post_boot, desired_ip_range=desired_ip_range)
        self.health_check()
...
class VmClusterNode(ClusterNode):
    ...
    def ensure_reboot(self) -> bool:
        ...
        common.wait_true(f"reboot {name}", vm_state, h=self.hostconn, node_name=name, running=False)

        r = self.hostconn.run(f"virsh start {name}")
        if not r.success():
            return False

        common.wait_true(f"reboot {name} finished", vm_state, h=self.hostconn, node_name=name, running=True)
