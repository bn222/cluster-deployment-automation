...
class BootcIsoBuilder:
    ...
    def ensure_image_is_built(self, image_name: str, build_action: Callable[[], None]) -> None:
        ...
        if not self.h.run(f"sudo podman pull {image_name}").success():
            logger.warning(f"Failed to pull remote image '{image_name}', trying to build it instead.")
            build_action()
            # Tag it as the expected remote name
            self.h.run(f"sudo podman tag containers-storage:{image_name} {image_name}")
