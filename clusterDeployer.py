...
class ClusterDeployer(BaseDeployer):
    ...
    def create_masters(self) -> None:
        ...
        image_futures = [(h.config.name, executor.submit(h.ensure_images, iso_file, infra_env, masters=True)) for h in hosts_with_masters]
        wait_futures("ensure image", image_futures, exit_on_error=False)
    ...
    def create_workers(self) -> None:
        ...
        image_futures = [(h.config.name, executor.submit(h.ensure_images, iso_file, infra_env, masters=False)) for h in hosts_with_workers]
        wait_futures("ensure image", image_futures, exit_on_error=False)
        ...
        wait_futures("install worker", nodes_with_futures, exit_on_error=False)
