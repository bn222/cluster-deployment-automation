...
class ClusterDeployer(BaseDeployer):
    def __init__(self, cc: ClustersConfig, ai: AssistedClientAutomation, steps: list[str], secrets_path: str, cdaConfig: CdaConfig, should_resume: bool = False):
        ...
        self.state = StateFile(cdaConfig)
        reset_state = self.state.state_dict.get("cluster-name") != cc.name or not should_resume
        if reset_state:
            logger.info("Resetting state file...")
            self.state.clear_state()
            self.state.dinsert("cluster-name", self._cc.name)
    ...
    def deploy(self) -> None:
        ...
        if self._cc.masters:
            if PRE_STEP in self.steps and self.state.not_deployed("pre-step"):
                ...
                self.state.dinsert("pre-step", "deployed")
            else:
            ...
            if self._cc.kind != "microshift":
                if (WORKERS_STEP in self.steps or MASTERS_STEP in self.steps) and self.state.not_deployed("workers"):
                    ...
                    self.state.dinsert("workers", "offline")
                if MASTERS_STEP in self.steps and self.state.not_deployed("masters"):
                    ...
                    self.state.dinsert("masters", "deployed")
                else:
                ...
                if WORKERS_STEP in self.steps and self.state.not_deployed("workers"):
                    ...
                    self.state.dinsert("workers", "deployed")
                else:
        ...
        if POST_STEP in self.steps and self.state.not_deployed("post-step"):
            ...
            self.state.dinsert("post-step", "deployed")
        else:
